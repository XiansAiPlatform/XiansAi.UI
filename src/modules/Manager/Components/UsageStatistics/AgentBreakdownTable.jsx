...
const AgentBreakdownTable = ({ data }) => {
  const { agentBreakdown, totalMetrics, unit, metricType } = data || {};
  ...
  const sortedData = useMemo(() => {
    ...
    return [...agentBreakdown].sort((a, b) => {
    ...
    return (
      <Box sx={{ textAlign: 'center', py: 4 }}>
        <Typography variant="body2" color="text.secondary">
          No agent breakdown data available
        </Typography>
      </Box>
    );
  }
              ...
              >
                ...
                <TableCell align="right">
                  <Typography variant="body2" fontWeight="medium">
                    {formatValue(agent.metrics.primaryCount)}
                  </Typography>
                </TableCell>
                ...
                {hasBreakdown && (
                  <>
                    <TableCell align="right">
                      <Typography variant="body2">
                        {formatNumber(agent.metrics.promptCount)}
                      </Typography>
                    </TableCell>
                    <TableCell align="right">
                      <Typography variant="body2">
                        {formatNumber(agent.metrics.completionCount)}
                      </Typography>
                    </TableCell>
                  </>
                )}
                ...
                {unit && (unit === 'ms' || unit === 'milliseconds' || unit === 'seconds') && (
                  <TableCell align="right">
                    <Typography variant="body2">
                      {agent.metrics.requestCount > 0
                        ? formatResponseTime(Math.round(agent.metrics.primaryCount / agent.metrics.requestCount))
                        : '-'}
                    </Typography>
                  </TableCell>
                )}
                ...
                <TableCell align="right">
                  <Typography variant="body2">
                    {formatNumber(agent.metrics.requestCount)}
                  </Typography>
                </TableCell>
              </TableRow>
            ))}
